  ...
  for(y2=0; y2 < length; y2++)
  {
    for(x2=0;x2 < width; x2++)
    {
      ...
      {        
        ...
        f_xy1 = matrix_valw(MatriceImg, x1, y1 + 1, length, width); // f(x, y+1)
        f_x1y1 = matrix_valw(MatriceImg, x1 + 1, y1 + 1, length, width); // f(x+1, y+1)
        f_xy = matrix_valw(MatriceImg, x1, y1, length, width); // f(x,y)
        f_x1y = matrix_valw(MatriceImg, x1 + 1, y1, length, width); // f(x+1, y)
...
int main(int argc,char **argv)
 {
  ...
  float** MatriceCorrelationC=fmatrix_allocate_2d(length,width);
  float** MatriceCorrelationIC=fmatrix_allocate_2d(length,width);
  float** MatriceInverse=fmatrix_allocate_2d(length, width);
  float** MatriceInverseI=fmatrix_allocate_2d(length, width);
  ...
  // Reset de la matrice image pivotee pour les calculs a venir
  for(i=0; i<length; i++)
  {
    for(j=0; j<width; j++)
    {
  ...
  // Pour implementer la formule (1), on suit les etapes suivantes:

  MultMatrix(MatriceCorrelationC, MatriceCorrelationIC, MatriceImg1, MatriceImgI1, MatriceImgPivotee, MatriceImgPivoteeI, length, width);

  CenterImg(MatriceCorrelationC, length, width);
  CenterImg(MatriceCorrelationIC, length, width);

  for(i=0; i<length; i++)
  {
    for(j=0; j<width; j++)
    {
      MatriceInverse[i][j] = MatriceCorrelationC[i][j] / (MatriceImg2[i][j] * MatriceImg2[i][j]);
      MatriceInverseI[i][j] = MatriceCorrelationIC[i][j] / (MatriceImgI2[i][j] * MatriceImgI2[i][j]);
    }
  }

  IFFTDD(MatriceInverse, MatriceInverseI, length, width);
  CenterImg(MatriceInverse, length, width);

  IFFTDD(MatriceInverse, MatriceInverseI, length, width);
  CenterImg(MatriceInverse, length, width);
  CenterImg(MatriceInverseI, length, width);

  FFTDD(MatriceInverse, MatriceInverseI, length, width);

  Recal(MatriceInverse, length, width);

  //Sauvegarde
  SaveImagePgm(NAME_IMG_OUT1,MatriceInverse,length,width);
